...
import { DEFAULT_SYSTEM_PROMPT, EMAIL_MANAGER_PROMPT, REPORT_GENERATOR_PROMPT } from '@/lib/prompts';
...
const agentConfig: Record<string, { name: string, greeting: string, placeholder: string }> = {
  'email-manager': {
    name: 'Email Manager',
    greeting: 'Hello! I am your Email Manager. Connect your Gmail to fetch recent emails, or tell me what kind of email you need to draft.',
    placeholder: 'Draft an email to...'
  },
  'auto-report-generator': {
    name: 'Auto-Report Generator',
    greeting: 'Hi! I am the Auto-Report Generator. Ask me to create a summary, format financial data, or generate a daily sales report in Indian formats.',
...
export default function AgentChatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (agentId === 'auto-report-generator') {
        systemInstruction = REPORT_GENERATOR_PROMPT;
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-6xl mx-auto">
      <div className="mb-6 flex items-center space-x-4">
        <Link href="/agents" className="p-2 rounded-full hover:bg-[var(--theme-background)] transition-colors">
          <ArrowLeft className="h-5 w-5 text-[var(--theme-muted)]" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[var(--theme-text)]">{config.name}</h1>
          <p className="text-[var(--theme-muted)] mt-1">Interact with your specialized AI co-pilot.</p>
        </div>
      </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={cn("flex items-start space-x-4", msg.role === 'user' ? "flex-row-reverse space-x-reverse" : "")}>
                ...
                <div className={cn(
                  ...
                  {msg.role === 'assistant' && agentId === 'auto-report-generator' && (
                    <ReportDownloader content={msg.content} />
                  )}
                  {msg.role === 'assistant' && agentId === 'email-manager' && i > 0 && (
